...
// 4. POST /api/:table — insert new record
...
// 5. PUT /api/:table/:id — update record
router.put('/:id', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `UPDATE ${table} SET ${setClauses} WHERE ${pk} = $${keys.length + 1} RETURNING *`,
      [...values, id]
    );
    if (rows.length === 0)
      return res.status(404).json({ error: `ID ${id} not found in ${table}` });
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
